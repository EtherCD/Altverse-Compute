  ...
  fn interact(&mut self, player: &mut crate::units::player::Player) {
    ...
    if player.pos.x - player.radius > 0.0 && player.pos.x + player.radius < self.entity.boundary.w {
      ...
      if dist <= self.entity.aura && !player.downed {
        ...
        let rot_mult = 1.0 - dist / self.entity.aura;
        angle += 0.005 * (1.0 + rot_mult * 2.0);

        let suck_strength = 0.6;
        let new_dist = (dist - suck_strength * (1.0 - dist / self.entity.aura)).max(5.0);

        player.pos.x = self.entity.pos.x + new_dist * angle.cos();
        player.pos.y = self.entity.pos.y + new_dist * angle.sin();
      }
    }
  }
